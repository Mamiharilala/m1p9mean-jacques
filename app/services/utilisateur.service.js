...
const getBeneficeRestaurant = async function (id) {
    try {
        let ids = mongoose.Types.ObjectId(id);
        var data = await Benefice.aggregate([
            { "$match": { "id_restaurant": ids } },
            {
                $group:
                {
...
const getBeneficeParJour = async function () {
    try {
        var data = await Benefice.aggregate(
            [
                {
...
const getBeneficeParRestaurant = async function () {
    try {
        var data = await Benefice.aggregate(
            [
                {
...
const getPlat = async function (id_plat) {
    try {
        var plat = await Plat.find({ _id: id_plat }).exec();
        return plat;
    } catch (e) {
...
const createCommande = async function (utilisateur, plat, quantite) {
    try {
        var commande = new Commande({
            "id_utilisateur": utilisateur.id, "id_plat": plat.id, "id_livreur": null,
